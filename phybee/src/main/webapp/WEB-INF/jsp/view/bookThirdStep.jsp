...
<form method="post" action="<c:url value="/reservation/redirection"/>">
	<input type="hidden" name="schedule" value="${schedule.getId()}" /> <input
		type="hidden" name="adult" value="${adult}" /> <input type="hidden"
		name="child" value="${child}" /> <input type="hidden" name="disabled"
		value="${disabled}" />
	<div class="row">
		<a
			href="<c:url value="/reservation/ticket?schedule=${schedule.getId()}"/>"
			class="link btn btn-primary"><c:out value="Return" /></a> <input
			type="submit" value="Proceed" class="btn btn-primary">
	</div>
</form>