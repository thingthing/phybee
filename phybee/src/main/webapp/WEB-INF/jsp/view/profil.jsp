...
<a href="<c:url value="/changePwd"/>"><spring:message
		code="account.changePwd" /></a>
<br />
<br />
		...
		<c:choose>
			...
			<c:when test="${not empty listmovie}">
				<c:forEach var="list" items="${listmovie}">
					<tr>
						<td><a
							href="<c:url value="/moviedetails?movie=${list.movie.mId}"/>"
							class="link"> <img class="img-rounded"
								alt="${list.movie.mTitle}"
								style="max-width: 110px; max-height: 145px;"
								src="<c:url value="/resources/poster/${list.movie.mPoster}"/>"></a>
						</td>
						<td><a
							href="<c:url value="/moviedetails?movie=${list.movie.mId}"/>"
							class="link">${list.movie.mTitle}</a></td>
						<td>${list.date}</td>
						<td>${list.start}</td>
						<td>${list.end}</td>
